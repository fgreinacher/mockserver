...
git tag mockserver-testcontainers/go/v8.0.0

# 2. Push the tag to GitHub
git push origin mockserver-testcontainers/go/v8.0.0

# 3. Request the Go module proxy to index it (optional — happens on first `go get`)
GOPROXY=https://proxy.golang.org GO111MODULE=on \
  go list -m github.com/mock-server/mockserver-monorepo/mockserver-testcontainers/go@v8.0.0
```
