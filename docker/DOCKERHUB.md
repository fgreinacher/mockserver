...
- bare version: `mockserver/mockserver:8.0.0`
- `mockserver-` prefixed: `mockserver/mockserver:mockserver-8.0.0`

`latest` always points at the most recent release of the default variant. **Pin to an explicit version (e.g. `8.0.0`) in production**; `latest`/`snapshot` are moving tags.
...
services:
  mockServer:
    image: mockserver/mockserver:8.0.0
    ports:
      - 1080:1080
    environment:
