...
public class MqttTlsHandshakeLiveBrokerIntegrationTest {
    ...
    public static void checkDockerAndStartTlsMosquitto() throws Exception {
        ...
        // --- Generate the broker's CA + server certificate (SAN localhost / 127.0.0.1) ---
        ...
        X509Certificate serverCertificate =
            generateServerCertificate(serverKeyPair, brokerCaCertificate, caKeyPair.getPrivate());
        ...
        sslBrokerUrl = "ssl://" + mosquitto.getHost() + ":" + mosquitto.getMappedPort(MQTT_TLS_PORT);
    }
    ...
    private static X509Certificate generateServerCertificate(KeyPair serverKeyPair,
        ...
        // SAN must match the host the client connects to (Testcontainers exposes on localhost /
        // 127.0.0.1); Paho enables HTTPS endpoint identification by default, which verifies it.
