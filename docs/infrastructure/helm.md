...
|-------|------|---------|---------|
| `mockserver` | `helm/mockserver/` | 8.0.0 | Main deployment chart (includes optional ConfigMap) |
| `mockserver-config` | `helm/mockserver-config/` | 8.0.0 | Example external ConfigMap chart (for reference) |
...
# Pin a version
helm install mockserver oci://ghcr.io/mock-server/charts/mockserver --version 8.0.0
...
> `GetSecretValue`) or the probe fails and signing is silently skipped — the cause of the 8.0.0 chart
