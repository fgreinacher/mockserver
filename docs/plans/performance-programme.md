...
**The cause is not client CPU — it is `sweep.js` violating this plan's own Finding 3.** My first
write-up of this said the fix was "more k6 cores, a second generator host". That was wrong, and
the run's own numbers refute it: at the 8,000 rung k6 used **190.5% of a 1,400% pin — 13.6%
utilisation, 86% idle**. The client had CPU to spare at every rung.
...
`sweep_client_had_headroom` — the validity check that failed build 322. The
["12 calibration" row](#what-remains) explains that failure as a deliberately short 3-rung
ladder putting every rung "at or above the k6 client's capability". That is the right effect
and the wrong mechanism: the binding constraint is the VU ramp, not client capability. The
published knee figures derived from this sweep deserve re-examination for the same reason.

**What would actually resolve it:** equalise `K6_SWEEP_PRE_VUS` and `K6_SWEEP_MAX_VUS` per
Finding 3, sized by Little's law for the target rung rather than left as a ramp, and add finer
ladder rungs between 4,000 and 16,000 — the current octave steps jump straight past the knee.
