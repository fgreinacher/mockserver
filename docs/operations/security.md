...
| `com.azure:azure-storage-blob` | 12.35.0 | `mockserver-blob-azure` | Azure Blob Storage client for the Azure-backed `BlobStore`. Supports Azurite emulator for testing. Pulls `azure-core-http-netty:1.16.5` → `reactor-netty-http`/`reactor-netty-core` `1.2.18` (matched pair, fixes the reactor-netty chained-redirect credential-leak advisory patched in `1.2.8`). The module pins `io.projectreactor:reactor-core` to `3.7.19` to converge the 12.35.0 stack's internal off-by-one (azure-core declares `3.7.18`, reactor-netty declares `3.7.19`). | Yes (targets Java 8+; the whole stack is Java-8 bytecode) |
