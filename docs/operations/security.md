...
- **For maximum security:** Pin to a specific release version (e.g., `7.5.0`), not `latest` or `SNAPSHOT`
- **For Renovate/Dependabot users:** Configure version constraints to only match release versions, not SNAPSHOTs
- **For Docker users:** Use versioned tags (e.g., `mockserver/mockserver:7.5.0`) rather than `latest`
