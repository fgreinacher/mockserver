...
The **read / matching** path (`firstMatchingExpectation`, `firstMatchingEarlyExpectation`, the `retrieve*` family and `size()`) is deliberately **not** synchronized — it relies on the queue's eventually-consistent `toSortedList()` snapshot — so request throughput is unaffected by control-plane writes.
