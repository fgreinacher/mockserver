...
| TLS inbound — dynamic | `certificateAuthorityPrivateKey`, `certificateAuthorityCertificate`, `dynamicallyCreateCertificateAuthorityCertificate`, `directoryToSaveDynamicSSLCertificate`, `preventCertificateDynamicUpdate`, `sslCertificateDomainName`, `sslSubjectAlternativeNameDomains`, `sslSubjectAlternativeNameIps`, `maxSubjectAlternativeNames` (cap on retained dynamically-discovered SAN entries; default `100`, FIFO eviction, configured/default SANs exempt — see [tls-and-security.md](tls-and-security.md#certificate-authority)), `sslCertificateLeafValidityInDays` (validity in days of the auto-generated **leaf** server certificate; default `397` to stay inside Apple's 825-day server-certificate cap — the old 10-year leaf exceeded it and broke Apple clients (#2531); raise to e.g. `3650` to restore the historical long-lived leaf; a non-positive value falls back to the default; the generated CA is unaffected and keeps its long life — see [tls-and-security.md](tls-and-security.md#certificate-authority)) |
...
| TLS outbound | `forwardProxyTLSX509CertificatesTrustManagerType`, `forwardProxyTLSHostnameVerificationEnabled` (verify upstream host name for JVM/CUSTOM trust — see [tls-and-security.md](tls-and-security.md#forward-proxy-trust)), `forwardProxyTLSCustomTrustX509Certificates`, `forwardProxyPrivateKey`, `forwardProxyCertificateChain`, `forwardProxyClientCertificatesByHost` (per-host outbound mTLS cert/key map — see [tls-and-security.md](tls-and-security.md#per-host-outbound-mtls)) |
| Protocol selection | `tlsProtocols` (default `TLSv1.2,TLSv1.3`), `tlsAllowInsecureProtocols` (default `false`; honour deprecated TLSv1/TLSv1.1), `proactivelyInitialiseTLS`, `useBouncyCastleForKeyAndCertificateGeneration`, `useSemicolonAsQueryParameterSeparator` |
| MCP | `mcpEnabled` |
...
Controls when `RequestMatchers` switches from the full linear scan of the `CircularPriorityQueue` to the `CandidateIndex` acceleration structure. It gates **both** `firstMatchingExpectation` (the matching path) and `clear(RequestDefinition)` (which would otherwise run a full match against every registered expectation). When the number of active expectations is **less than** this threshold, the untouched linear scan always runs (no change in behaviour or performance at any expectation count below the threshold). When the expectation count **reaches or exceeds** the threshold, the candidate index engages and narrows the scan to a `(method, path)` bucket union the fallthrough list (see [CandidateIndex](request-processing.md#candidateindex--matching-acceleration-for-large-expectation-sets) in `request-processing.md`).
...
> **Test hygiene — `ConfigurationProperties` caches resolved values.** `readPropertyHierarchically` caches **every** value it resolves, including built-in defaults, in the static `propertyCache`, and every getter reads cache-first. A raw `System.setProperty(...)` / `System.clearProperty(...)` in a test does **not** touch that cache, so once any sibling or co-resident test has read a key (caching a default), a later raw mutation of that key is silently ignored — the getter keeps returning the cached value. This is ordering-dependent, so it passes locally and fails only under CI's method ordering (it has cost a red `master`). Only the private cache-coherent `setProperty`/`clearProperty` helpers (used by the fluent setters) keep the cache in step. In tests, either drive state through the fluent `ConfigurationProperties.<name>(value)` setter (cache-coherent), or, when asserting raw system-property resolution, evict the cache entry through the same reflection helper the existing config tests use — see the `clearCacheEntry` / `clearProgrammaticallySetKey` pattern in `PrometheusRemoteWriteProtocolVersionConfigurationTest`, `SslCertificateLeafValidityInDaysTest`, and `MaxLlmConversationBodySizeTest`. Any test that mutates this global static store must also be registered in **both** the parallel-exclude and sequential-include lists in `mockserver-core/pom.xml`.
