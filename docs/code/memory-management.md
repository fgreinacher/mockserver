...
Actual heap retention is a small multiple of this figure (headers, metadata, UUID strings, etc.), so the default budget deliberately sits at a fraction of the heap-ceiling budget rather than the whole of it. The fraction is log-level-aware: one-eighth at `INFO`/`DEBUG`/`TRACE` and one-quarter at `WARN`/`ERROR`/`OFF`. The tighter INFO divisor compensates for message memoisation: at `INFO`, `MockServerEventLog.processLogEntry()` calls `writeToSystemOut()` immediately after adding an entry to the deque, which calls `entry.getMessage()` and stores the formatted string on the entry for its whole life in the deque. For a JSON-body workload, the formatted message includes the request body rendered as text, so it is roughly as large as the raw body bytes the weigher counts — measured at **~4.8× the weigher total at `INFO`** versus **~2.6× at `WARN`** on 20,000 retained entries of ~1 KB JSON bodies (HprofHisto shallow bytes: 101 MB and 54 MB respectively; weigher total 21.2 MB for both). The ~1.9× INFO/WARN ratio in actual heap is what the 2× tighter divisor compensates for. At `WARN`, the majority of `INFO`-level request/response entries are never written to system-out (the `isEnabled(INFO, WARN)` guard on `MockServerLogger.writeToSystemOut()` returns false), so `getMessage()` is not called and the message string is not memoised. For a 2 GB heap the default budget is around 480 MB at `WARN` and 240 MB at `INFO` — but those are **counted** body bytes, not retained heap. Apply the ratios above and the same defaults correspond to roughly 1.2 GB actually retained at `WARN` and 1.15 GB at `INFO`, i.e. both levels land near 60% of a 2 GB heap once the log is full. Size against the retained figure, not the budget: the budget is the dial, the multiple is what the heap sees. The **same budget also bounds the ring buffer's in-flight backlog** (see [Ring In-Flight Bounding and Drops](#ring-in-flight-bounding-and-drops)), so tightening it at rendering levels constrains both the deque and the ring simultaneously. Lower it if the log shares the heap with a large expectation store or heavy forwarding; raise it if you have headroom and want to retain more history.
