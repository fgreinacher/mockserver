...
> **This has already gone stale once.** When `grpcBidiResponse.matchJson` and
> `httpWebSocketResponse.matchers[].textMatcher` moved to `stringOrJsonSchema` (so a matcher value
> starting with `!` could be expressed unambiguously), the editor schemas were not regenerated. Both
> still declared `{"type": "string"}`, so VS Code and JetBrains flagged **valid** input as an error —
> the editor and the server disagreeing about what is legal, which is the failure this schema exists
> to prevent.
>
> **Recommended (not yet implemented): a CI drift check.** Regenerate into a temporary location and
> fail if it differs from the committed copies. Note the trap when wiring it: the natural home is the
> `mockserver-editors` pipeline, but that is triggered by `^(mockserver-vscode/|mockserver-jetbrains/)`,
> and the change that invalidates the schema is a **`mockserver-core` schema edit** — so a
> path-filtered gate there would never see the commit that breaks it. Either trigger the editors
> pipeline on `mockserver/mockserver-core/src/main/resources/org/mockserver/model/schema/` as well, or
> emit the check unconditionally the way the client version-pin guard is emitted from
