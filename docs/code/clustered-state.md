...
`reconcileFromBackend()` runs under the `RequestMatchers` instance monitor (`synchronized`), and so does **every** node-local cache mutator (`add`, `update`, `reset`, the `clear` family, `removeHttpRequestMatcher`). This is a genuine single-writer contract enforced internally, not an assumption about external serialization: local control-plane adds race across the `nioEventLoopThreadCount` Netty worker threads, and a clustered backend fires its invalidation callback (`InvalidationListener.onChanged/onCleared → reconcileFromBackend`) from its own threads. Sharing one reentrant monitor means the reconcile never observes a half-applied local mutation and vice-versa. Without it (issue #2579), a concurrent add's local-then-backend-then-trim sequence let one thread's eviction trim delete another thread's not-yet-persisted matcher, silently dropping an acknowledged expectation. The read/matching path is intentionally left outside this monitor and relies on the queue's eventually-consistent sorted snapshot.
