...
#### Documentation
- **AI/LLM/MCP and load-injection documentation review — clarity, value, and discoverability.** Added an "AI & MCP Overview" landing page that routes readers by use case (MockServer controlled by AI, mocking AI services, or observing AI traffic); cross-listed LLM Response Mocking into the AI & MCP section; led the AI pages with value-first openings and added "how it works" diagrams. Grouped the three performance pages (serving / driving / injecting) under Performance Testing with a shared signpost table and a "Load Scenarios vs k6/Gatling" positioning. For SEO and AI answer-engines: added FAQ structured data (FAQPage JSON-LD) to eight feature pages, tightened titles/meta descriptions, and corrected stale `llms.txt` entries.
- **"How it works" diagrams now render in the MockServer house style.** The website has no Mermaid renderer, so the AI / load-injection "how it works" diagrams were converted to house-style PNGs (white boxes, dashed Expectation/Proxy boxes, blue arrows) on eight pages — load injection, AI traffic inspection, AI debugging, AI protocol mocking, AI overview, contract verification, LLM cost optimisation, and centralised deployment. Each diagram is also an editable slide in `images/MockServerScenarios.pptx`, generated by additive tooling in `images/diagram-tools/` (`build_ai_diagrams.py`, `render_ai_architecture.py`) that never overwrites the existing slides.
