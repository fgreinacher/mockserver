...
| `sweep.js` | Throughput-vs-latency "knee" curve. Offers the match path (`GET /simple`) at an ascending LADDER of fixed arrival rates (`K6_SWEEP_RATES`); each rate is a staggered `constant-arrival-rate` step tagged `rate:<offered>`. Per step it records the ACHIEVED throughput, latency percentiles (p50/p90/p95/p99/p99.9), and error rate. Writes `{proto, points:[{offered_rps, achieved_rps, p50_ms…p999_ms, sample_count, error_rate}]}` to `K6_SWEEP_RESULT_PATH` — the series plotted as a load-vs-latency knee curve on the docs site. Also REUSED per pinned core-count by the serving per-core harness (`.buildkite/scripts/steps/lib/perf-percore.sh`, performance-programme item 18); `sample_count` is the per-rung completed-request count it needs for MIN_TAIL_SAMPLES tail suppression. | Notify-only (NO aborting thresholds — dropped iterations / degradation at the top of the ladder are the point). |
